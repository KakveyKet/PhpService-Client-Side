...
const filteredItems = computed(() => {
  return items.value.filter((item) => {
    ...
    return (
      matchesCustomerSearch(item.customerId, filterSearch.value) &&
...
<template>
  <div>
      ...
      >
        <template #empty>
          <div class="empty-state">
            <i class="pi pi-file-edit" />No applications found.
          </div>
        </template>
          ...
          <div v-if="!auth.isCustomer" class="form-field form-field--full">
            ...
            >
              <template #option="{ option }"
                >{{ fullName(option) }} — {{ option.customerCode }}</template
              >
    ...
    <Dialog
      ...
      modal
      :header="`${reviewForm.decision === 'APPROVED' ? 'Approve' : 'Reject'} application`"
      :style="{ width: '560px', maxWidth: '95vw' }"
    >
        ...
        <div v-if="selected" class="mb-5 grid gap-3 sm:grid-cols-3">
          ...
          <div class="rounded-xl bg-slate-50 p-3">
            ...
            <strong class="mt-1 block text-sm text-slate-900">
              {{
                currency(
                  selected.monthlyIncome ?? selected.customerId?.monthlyIncome,
                )
              }}
            </strong>
          </div>
          <div class="rounded-xl bg-slate-50 p-3">
            ...
            <strong class="mt-1 block break-words text-sm text-slate-900">
              {{ selected.customerId?.occupation || "—" }}
            </strong>
          </div>
        </div>
        <div class="form-grid">
