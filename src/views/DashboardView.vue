...
const quickAccessItems = [
  ...
  {
    label: "Reports",
    description: "View the loan approval report",
  ...
  <div>
    ...
    <template v-else-if="auth.isCustomer">
      <div class="stats-grid">
        <StatCard
          label="Applications"
          :value="dashboard.applicationCount || 0"
          icon="pi pi-file-edit"
          tone="blue"
        />
        <StatCard
          label="Active loans"
          :value="dashboard.activeLoanCount || 0"
          icon="pi pi-wallet"
        />
        <StatCard
          label="Outstanding"
          :value="currency(customerOutstanding)"
          icon="pi pi-chart-line"
          tone="amber"
        />
        <StatCard
          label="Next payment"
          :value="currency(dashboard.nextInstallment?.remainingDue)"
          icon="pi pi-calendar"
          tone="rose"
          :note="date(dashboard.nextInstallment?.dueDate)"
        />
      </div>
      <section class="table-card">
        <div class="panel-card__header">
          ...
          <RouterLink to="/applications"
            ><Button label="Request loan" icon="pi pi-plus" size="small"
          /></RouterLink>
        </div>
        <DataTable
        ...
        >
          <template #empty
            ><div class="empty-state">
              <i class="pi pi-wallet" />No loans yet.
            </div></template
          >
          ...
          <Column header="Balance"
            ><template #body="{ data }"
              ><strong>{{ currency(data.balances?.total) }}</strong></template
            ></Column
          >
    ...
    <template v-else-if="!loading">
      <div class="stats-grid">
        ...
        <StatCard
          ...
          :value="dashboard.pendingApplicationCount || 0"
          icon="pi pi-file-edit"
          tone="blue"
        />
        <StatCard
          label="Portfolio balance"
          :value="currency(dashboard.portfolioBalance)"
          icon="pi pi-wallet"
          tone="amber"
        />
        <StatCard
          label="Today's collections"
          :value="currency(dashboard.todayCollections)"
          icon="pi pi-receipt"
          tone="rose"
        />
      </div>
      ...
      <div class="content-grid">
        <section class="table-card">
          ...
          <DataTable
            :value="dashboard.recentApplications || []"
            striped-rows
            responsive-layout="scroll"
          >
            <template #empty
              ><div class="empty-state">
                <i class="pi pi-file" />No applications found.
              </div></template
            >
        ...
        <section class="panel-card">
          ...
          <div class="grid gap-3 p-4">
            ...
            >
              <span
                class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700 transition group-hover:bg-emerald-600 group-hover:text-white"
              >
                <i :class="item.icon" />
              </span>
              ...
              <i
                class="pi pi-chevron-right text-xs text-slate-400 transition group-hover:translate-x-0.5 group-hover:text-emerald-700"
              />
            </RouterLink>
          </div>
        </section>
